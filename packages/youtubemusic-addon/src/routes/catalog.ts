...
import type { CatalogPage, HomeItem, HomeSection, QuickAccessItem, YouTubeMusicConfig } from "../types";
import { PROVIDER_ID } from "../utils";
import { fetchBatchTrackMetadata } from "./queue";

export async function handleHome(config: YouTubeMusicConfig, continuation?: string): Promise<CatalogPage> {
  try {
    ...
    if (continuation) {
      ...
      const data = await ytFetch("browse", config, { continuation });
      ...
      const data = await ytFetch("browse", config, { browseId: "FEmusic_home" });
      ...
      while (nextContinuation && contAttempts < 10) {
        contAttempts++;
        try {
          const contData = await ytFetch("browse", config, { continuation: nextContinuation });
      ...
      if (quickAccess && quickAccess.length > 0) {
        ...
        const meta = await fetchBatchTrackMetadata(config, trackIds);
