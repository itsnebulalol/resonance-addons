...
import type { LyricsData, LyricsLine, YouTubeMusicConfig } from "../types";
...
export async function handleLyrics(
  ...
  try {
    ...
    if (!targetVideoId && title) {
      ...
      const searchData = await ytFetch("search", config, { query, params: "EgWKAQIIAWoKEAkQBRAKEAMQBA==" });
    ...
    const lyrics = await fetchLyrics(config, targetVideoId);
    return lyrics;
  } catch (e: any) {
...
async function fetchLyrics(config: YouTubeMusicConfig, videoId: string): Promise<LyricsData | null> {
  try {
    const nextData = await ytFetch("next", config, { videoId, isAudioOnly: true });
    ...
    const browseData = await ytFetch("browse", config, { browseId: lyricsId });
