...
export async function handleRelated(config: YouTubeMusicConfig, browseId: string): Promise<Track[]> {
  try {
...
export async function handleRelatedForTrack(config: YouTubeMusicConfig, videoId: string): Promise<Track[]> {
  try {
    ...
    return handleRelated(config, relatedBrowseId);
  } catch (e: any) {
