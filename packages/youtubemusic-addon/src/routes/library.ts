...
async function fetchPageRenderers(
  ...
  if (continuationToken) {
    const page = await ytFetch("browse", config, { continuation: continuationToken });
  ...
  const firstPage = await ytFetch("browse", config, { browseId });
...
function containsPlaylistEditEndpoint(value: any): boolean {
  if (value == null || typeof value !== "object") return false;
  if ("playlistEditEndpoint" in value) return true;
  return Object.values(value).some(containsPlaylistEditEndpoint);
}

function parsePlaylists(renderers: any[]): HomeItem[] {
  const items: HomeItem[] = [];
  const editableAuthors = new Set(
    renderers
      .filter(containsPlaylistEditEndpoint)
      .map((renderer) =>
        String((renderer.subtitle?.runs ?? []).map((run: any) => run.text).join(""))
          .split(" • ")[0]
          ?.trim(),
      )
      .filter(Boolean),
...
export async function handleLibrary(
  ...
  try {
    if (type) {
      ...
      if (!browseId) throw new AddonError(`Invalid library type: ${type}`, 400);

      let section: HomeSection;
      if (type === "songs") {
        const { items, continuationToken } = await fetchPageShelfItems(config, browseId, continuation);
        section = {
        ...
        const { renderers, continuationToken } = await fetchPageRenderers(config, browseId, continuation);
      ...
      return { sections: [section], filters: [], quickAccess: null, continuation: null } satisfies CatalogPage;
    }

    const [playlistPage, songPage, albumPage, artistPage] = await Promise.all([
      fetchPageRenderers(config, "FEmusic_liked_playlists"),
      fetchPageShelfItems(config, "FEmusic_liked_videos"),
      fetchPageRenderers(config, "FEmusic_liked_albums"),
      fetchPageRenderers(config, "FEmusic_library_corpus_track_artists"),
    ]);
    ...
    return { sections, filters: [], quickAccess: null, continuation: null } satisfies CatalogPage;
  } catch (e: any) {
