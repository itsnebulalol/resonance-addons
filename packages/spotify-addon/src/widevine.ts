...
const b2s = (b: Buffer | Uint8Array) => { let s = ""; for (let i = 0; i < b.length; i++) s += String.fromCharCode(b[i]!); return s; };
const s2b = (s: string) => { const o = Buffer.alloc(s.length); for (let i = 0; i < s.length; i++) o[i] = s.charCodeAt(i) & 0xff; return o; };
const u16be = (b: Buffer, o: number) => ((b[o]! << 8) | b[o + 1]!) >>> 0;
const bufEq = (a: Buffer, b: Buffer) => { if (a.length !== b.length) return false; for (let i = 0; i < a.length; i++) if (a[i] !== b[i]) return false; return true; };

function encVarint(n: number | bigint): Buffer {
  let v = BigInt(n); const o: number[] = [];
  while (v > 0x7fn) { o.push(Number(v & 0x7fn) | 0x80); v >>= 7n; }
  o.push(Number(v & 0x7fn)); return Buffer.from(o);
}
...
function parseFields(buf: Buffer): Record<number, (Buffer | bigint)[]> {
  ...
  const rv = () => { let shift = 0n, r = 0n, b: number; do { b = buf[i++]!; r |= BigInt(b & 0x7f) << shift; shift += 7n; } while (b & 0x80); return r; };
  while (i < buf.length) {
    const t = Number(rv()), field = t >> 3, wire = t & 7; let val: any;
    if (wire === 0) val = rv();
    else if (wire === 2) { const len = Number(rv()); val = buf.subarray(i, i + len); i += len; }
...
function ecbEnc(key16: Buffer, block16: Buffer): Buffer {
  ...
  c.start(); c.update(forge.util.createBuffer(b2s(block16))); c.finish();
  return s2b(c.output.getBytes()).subarray(0, 16);
}
function xorB(a: Buffer, b: Buffer): Buffer { const o = Buffer.alloc(a.length); for (let i = 0; i < a.length; i++) o[i] = a[i]! ^ b[i]!; return o; }
function dbl(buf: Buffer): Buffer {
  const o = Buffer.alloc(16); let carry = 0;
  for (let i = 15; i >= 0; i--) { o[i] = ((buf[i]! << 1) & 0xff) | carry; carry = (buf[i]! & 0x80) ? 1 : 0; }
  if (buf[0]! & 0x80) o[15]! ^= 0x87;
  return o;
}
function cmac(key: Buffer, msg: Buffer): Buffer {
  const L = ecbEnc(key, Buffer.alloc(16)), K1 = dbl(L), K2 = dbl(K1);
  ...
  else { const rem = msg.subarray((n - 1) * 16); const padded = Buffer.concat([rem, Buffer.from([0x80]), Buffer.alloc(16 - rem.length - 1)]); last = xorB(padded, K2); }
...
function loadDevice(wvd: Buffer) {
  ...
  let o = 7;
  const pkLen = u16be(wvd, o); o += 2; const privKeyDer = wvd.subarray(o, o + pkLen); o += pkLen;
  const cidLen = u16be(wvd, o); o += 2; const clientId = wvd.subarray(o, o + cidLen);
...
function pubFromPkcs1(der: Buffer) {
  ...
  const n = asn1.value[0].value, e = asn1.value[1].value;
...
interface ServiceCert { providerId: string; serial: Buffer; pub: any; }
...
export class WidevineCDM {
  ...
  setServiceCertificate(certBytes: Buffer): string {
    ...
    const drmCert = parseFields(sdc[1][0] as Buffer);      // DrmCertificate
    this.cert = {
      providerId: (drmCert[7][0] as Buffer).toString("utf8"),
      serial: drmCert[2][0] as Buffer,
      pub: pubFromPkcs1(drmCert[4][0] as Buffer),
    };
  ...
  getChallenge(initData: Buffer): Buffer {
    ...
    cc.start({ iv: b2s(privacyIv) }); cc.update(forge.util.createBuffer(b2s(this.d.clientId))); cc.finish();
    ...
    const eci = Buffer.concat([
      fStr(1, this.cert.providerId), fBytes(2, this.cert.serial),
      fBytes(3, encClientId), fBytes(4, privacyIv), fBytes(5, encPrivacyKey),
    ]);

    let requestId: Buffer;
    if (this.d.type === 2) {
      const rnd = s2b(forge.random.getBytesSync(4));
      const counter = Buffer.alloc(8); counter[0] = 1;             // session number, little-endian
    ...
    const md = forge.md.sha1.create(); md.update(b2s(msg));
  ...
  parseLicense(licenseBytes: Buffer): ContentKey {
    ...
    const licMsg = sm[2][0] as Buffer, sessionKeyEnc = sm[4][0] as Buffer;
    const sessionKey = s2b((this.d.privateKey as any).decrypt(b2s(sessionKeyEnc), "RSA-OAEP", { md: forge.md.sha1.create(), mgf1: { md: forge.md.sha1.create() } }));
    ...
    for (const kc of (lic[3] as Buffer[] | undefined) || []) {
      ...
      dec.start({ iv: b2s(f[2][0] as Buffer) }); dec.update(forge.util.createBuffer(b2s(f[3][0] as Buffer))); dec.finish();
...
export function psshInitData(psshBox: Buffer): Buffer {
  ...
  if (ver > 0) { const kc = psshBox.readUInt32BE(o); o += 4 + 16 * kc; }
  const dataSize = psshBox.readUInt32BE(o); o += 4;
