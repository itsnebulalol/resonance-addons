import { albumToSearch, ampGet, artistToSearch, artworkURL, getStorefront, msToDuration, songToTrack } from "../amapi";
...
export async function handleAlbum(id: string): Promise<AlbumDetail> {
  ...
  const path = isLib(id, "l.")
    ? `/v1/me/library/albums/${id}`
    : `/v1/catalog/${sf}/albums/${id}`;
...
export async function handlePlaylist(id: string): Promise<PlaylistDetail> {
  const sf = await getStorefront();
  const path = isLib(id, "p.")
    ? `/v1/me/library/playlists/${id}`
    : `/v1/catalog/${sf}/playlists/${id}`;
  const d = await ampGet(path, { include: "tracks", "limit[tracks]": 100 });
  const pl = d?.data?.[0];
...
export async function handleArtist(id: string): Promise<ArtistDetail> {
  ...
  if (isLib(id, "r.")) {
    ...
    return {
      id: String(id), name: ar?.attributes?.name ?? "", thumbnailURL: null, subtitle: null,
      topTracks: [], albums, singles: [], playlists: [], relatedArtists: [],
    };
  }
