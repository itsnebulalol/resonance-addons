...
export class WidevineCDM {
  ...
  getChallenge(kid: Buffer): Buffer {
    ...
    if (this.d.type === 2) {
      ...
      const sess = Buffer.alloc(8); sess[0] = 1; // writeUInt32LE(1,0) == [01,00,00,00,..] in a zeroed buffer
      requestId = Buffer.from(Buffer.concat([rnd, Buffer.alloc(4), sess]).toString("hex").toUpperCase(), "latin1");
    ...
    const msg = Buffer.concat([
      fBytes(1, this.d.clientId), fBytes(2, contentId), fVarint(3, 1 /*NEW*/),
      fVarint(4, Math.floor(Date.now() / 1000)), fVarint(6, 21 /*VERSION_2_1*/), fVarint(7, nonce),
    ]);
  ...
  parseLicense(licenseBytes: Buffer): ContentKey {
    ...
    const hm = forge.hmac.create(); hm.start("sha256", b2s(macServer)); hm.update(b2s(Buffer.concat([oem, licMsg])));
