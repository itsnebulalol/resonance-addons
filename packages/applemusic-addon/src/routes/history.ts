...
import { ampGet, getStorefront } from "../api";
...
export async function handleHistory(trackId: string, event: HistoryEvent): Promise<void> {
  try {
    ...
    try {
      const songData = await ampGet(`/v1/catalog/${storefront}/songs/${trackId}`, {
        include: "albums",
      });
    ...
    const common = {
      ...
      "container-type": albumId ? 3 : 0,
      ...(albumId ? { "container-ids": { "album-adam-id": albumId } } : {}),
      "developer-token": developerToken,
      "feature-name": albumName ?? "music_kit-integration",
    ...
    const events = [
      {
        ...common,
        "event-reason-hint-type": albumId ? 1 : 0,
        "event-type": 1,
