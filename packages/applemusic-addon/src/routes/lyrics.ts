...
export async function handleLyrics(title?: string, artist?: string, _videoId?: string): Promise<LyricsData | null> {
  try {
    ...
    const lyrics = await fetchLyrics(result.songId, result.storefront);
    return lyrics;
  } catch (e: any) {
...
async function fetchLyrics(songId: string, storefront: string): Promise<LyricsData | null> {
  const syllable = await fetchTTML(songId, storefront, "syllable-lyrics");
  if (syllable) return syllable;

  const line = await fetchTTML(songId, storefront, "lyrics");
  if (line) return line;

  return null;
}

async function fetchTTML(songId: string, storefront: string, endpoint: string): Promise<LyricsData | null> {
  const token = await getDeveloperToken();
  const url = catalogURL(storefront, `/songs/${songId}/${endpoint}`);
